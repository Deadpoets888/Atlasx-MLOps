...
def apply_bow(train_data: pd.DataFrame, test_data: pd.DataFrame, max_features: int) -> tuple:
    ...
    try:
        ...
        X_train_bow = vectorizer.fit_transform(X_train) #Learns vocabulary from training data, convert each review into word count vector
        X_test_bow = vectorizer.transform(X_test)

        train_df = pd.DataFrame(X_train_bow.toarray())
        train_df['label'] = y_train #merge label(1, 0 ) qith train df
